...
static void test_execute_negative_null_store(void);
static void test_execute_negative_null_stack(void);
static void test_execute_negative_null_stream(void);
static void test_execute_negative_missing_prod(void);


void test_execute_negative(void)
{
    RUN_TEST(test_execute_negative_null_store);
    RUN_TEST(test_execute_negative_null_stack);
    RUN_TEST(test_execute_negative_null_stream);
    RUN_TEST(test_execute_negative_missing_prod);
}

static void test_execute_negative_missing_prod(void)
{
    test_resmach_prod_config_t entry_cfg = { "entry(%{termial})", "Term" };
    ...
    TEST_ASSERT_EQUAL_STRING_MESSAGE("entry(%{termial})", buffer, "");
...
static void test_execute_negative_null_store(void)
{
    ...
    FILE *  stream = fmem_open(&fm, "w+");
    uint8_t retval = resmach_execute(NULL, &stack, stream);

    TEST_ASSERT_EQUAL_UINT8_MESSAGE(RESMACH_EXECUTE_FAIL | RESMACH_EXECUTE_NULL_STORE, retval, "");
    fmem_term(&fm);
}
...
static void test_execute_negative_null_stack(void)
{
    ...
    FILE *  stream = fmem_open(&fm, "w+");
    uint8_t retval = resmach_execute(&production_store, NULL, stream);

    TEST_ASSERT_EQUAL_UINT8_MESSAGE(RESMACH_EXECUTE_FAIL | RESMACH_EXECUTE_NULL_STACK, retval, "");
    fmem_term(&fm);
}
...
static void test_execute_negative_null_stream(void)
{
    ...
    uint8_t retval = resmach_execute(&production_store, &stack, NULL);

    TEST_ASSERT_EQUAL_UINT8_MESSAGE(RESMACH_EXECUTE_FAIL | RESMACH_EXECUTE_NULL_STREAM, retval, "");
    fmem_term(&fm);
}
